...
public class TerraFeatureMushroomGlowing extends Feature<NoneFeatureConfiguration>
{
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context)
    {
        ...
        return generateMushroom(worldgenlevel, pos, random);
    }
    ...
    public static boolean checkCapSpace(WorldGenLevel world, BlockPos pos, Random rand, int startingLayer)
    {
        ...
        for (int y = pos.getY(); y < pos.getY() + (treeHeight - (startingLayer - 1)); y++)
        {
            for (int x = pos.getX() - radius; x <= pos.getX() + radius; x++)
            {
                for (int z = pos.getZ() - radius; z <= pos.getZ() + radius; z++)
                {
                    ...
                    {
                        if (!world.getBlockState(target).isAir() && world.getBlockState(target).getBlock() != LOG_MUSHROOM.getBlock())
                        {
                            canGrow = false;
                        }
                    }
    ...
    public static void placeBlock(WorldGenLevel world, BlockPos pos, BlockState block)
    {
    ...
    public static boolean generateCap(WorldGenLevel world, BlockPos pos, Random rand, int startingLayer)
    {
        int        arrayX      = 0, arrayY = startingLayer - 1, arrayZ = 0;
    ...
    public static boolean generateMushroom(WorldGenLevel world, BlockPos pos, Random rand)
    {
        BlockPos.MutableBlockPos target = new BlockPos.MutableBlockPos();
        target.set(pos);
        ...
        {
            if (!world.getBlockState(pos.above(trunk)).isAir())
            {
                canSpawn = false;
            }
        }
            ...
            if (canSeeSky && trunkHeight > 2)
            {
                ...
                if (checkCapSpace(world, branchPos, rand, 4))
                {
                    ...
                    for (int branch = 1; branch <= branchLength; branch++)
                    {
                        ...
                        if (world.getBlockState(branchTarget).isAir() || world.getBlockState(branchTarget).getBlock() == LOG_MUSHROOM.getBlock())
                        {
