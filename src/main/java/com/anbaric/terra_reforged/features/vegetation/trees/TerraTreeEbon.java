...
public class TerraTreeEbon extends Feature<NoFeatureConfig>
{
    ...
    public TerraTreeEbon(Function<Dynamic<?>, ? extends NoFeatureConfig> configFactory)
    {
        super(configFactory);
    }
    ...
    private boolean isValidGround(BlockState state, IBlockReader worldIn, BlockPos pos)
    {
        ...
        return target == TerraBlockRegistry.GRASS_CORRUPT.get() || target == Blocks.DIRT || target == Blocks.GRASS_BLOCK;
    }

    private static char[][][] mutateArray(char[][][] inputArray, Random rand)
    {
        System.out.println("Successfully starting to mutate array");
    ...
    {
        System.out.println("Successfully started generating tree");
        int        arrayX      = 0, arrayY = 0, arrayZ = 0;
                    ...
                    for (int z = pos.getZ() - 2; z <= pos.getZ() + 2; z++)
                    {
                        ...
                        switch (inputChar)
                        {
                            ...
                            case 'W':
                                ...
                                break;
                            case 'v':
                                world.setBlockState(target, LOG_EBON.with(LogBlock.AXIS, Direction.Axis.X), 3);
                                break;
                            case 'h':
                                world.setBlockState(target, LOG_EBON.with(LogBlock.AXIS, Direction.Axis.Z), 3);
                                break;
                        }
                        arrayZ++;
                    }
