...
public class TerraTreeMahoganyGreat extends Feature<NoFeatureConfig>
{
    ...
    public TerraTreeMahoganyGreat(Function<Dynamic<?>, ? extends NoFeatureConfig> configFactory)
    {
        super(configFactory);
    }
    ...
    void readArray(char[][][] seed)
    {
        for (int i = 0; i < blocks.length; i++)
        {
            ...
            {
                char readIndex0 = seed[i][j][0];
                char readIndex1 = seed[i][j][1];
                char readIndex2 = seed[i][j][2];
                char readIndex3 = seed[i][j][3];
                char readIndex4 = seed[i][j][4];
                char readIndex5 = seed[i][j][5];
                System.out.println(readIndex0 + " " + readIndex1 + " " + readIndex2 + " " + readIndex3 + " " + readIndex4 + " " + readIndex5);
            }
        }
    }
    ...
    private boolean isValidGround(BlockState state, BlockPos pos)
    {
                    ...
                    for (int x = baseX - offset; x < baseX - offset + treeWdth; x++)
                    {
                        for (int z = baseZ - offset; z < baseZ - offset + treeWdth; z++)
                        {
                            ...
                            if (placerState.canBeReplacedByLogs(world, pos))
                            {
                                ...
                                {
                                    world.setBlockState(placerPos, LOG_MAHOGANY.with(LogBlock.AXIS, Direction.Axis.X), 3);
                                }
                                else if (indexChar == 'Z')
                                {
                                    world.setBlockState(placerPos, LOG_MAHOGANY.with(LogBlock.AXIS, Direction.Axis.Z), 3);
                                }
                            }
                            index3++;
                        }
    ...
    {
        if (world.getBlockState(pos).getMaterial().isReplaceable() && isValidGround(world.getBlockState(pos.down()), pos))
        {
